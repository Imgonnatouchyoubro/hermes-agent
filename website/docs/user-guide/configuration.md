...
**Precedence:** per-call `provider` (from `delegate_task` arguments) → `delegation.base_url` in config → `delegation.provider` in config → parent provider (inherited). Per-call `model` (from `delegate_task` arguments) → `delegation.model` in config → parent model (inherited). Setting just `model` without `provider` changes only the model name while keeping the parent's credentials (useful for switching models within the same provider like OpenRouter). Per-call `provider`/`model` are accepted on `delegate_task` — top-level as a default for every task in the call, or per-entry via `tasks[].provider` / `tasks[].model` to override the top-level value for that task. Only **named** providers (resolved through the runtime provider system / config) are accepted per-call; a per-call provider cannot be an arbitrary URL, and `base_url`/`api_key` are never taken from the model.
