...
def _apply_per_call_overrides(
    base_creds: dict, model: Optional[str] = None, provider: Optional[str] = None,
) -> dict:
    model = str(model or "").strip() or None
    provider = str(provider or "").strip() or None
    if provider:
        return _runtime_provider_credentials({"model": model, "provider": provider}, None)
    if model:
        return {**base_creds, "model": model}
    return base_creds
