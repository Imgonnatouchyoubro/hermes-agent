...
def _build_children(
    ...
    task_creds: Optional[List[dict]] = None,
    ...
    for i, t in enumerate(task_list):
        child_creds = task_creds[i] if task_creds is not None else creds
        overrides = {
...
def delegate_task(
    ...
    from tools.delegate_tool_config import _apply_per_call_overrides
    ...
    for i, task in enumerate(task_list):
        try:
            task_creds.append(_apply_per_call_overrides(
                creds, model=task.get("model") or top_model, provider=task.get("provider") or top_provider,
            ))
        except ValueError as exc:
...
_DESCRIPTION_TAIL = (
    ...
    "or per-call model/provider (tasks[].model/.provider overrides the call defaults)."
)
...
def _build_tasks_param_description() -> str:
    ...
    return (
        ...
        "is a one-entry array. Required when spawning. Each task may override model/provider."
    )
...
DELEGATE_TASK_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            "model": _p(
                "string", "Optional model name for all children, overriding delegation.model. "
                "Provider credentials still resolve from config or the parent; tasks[].model takes precedence.",
            ),
            "provider": _p(
                "string", "Optional named provider for all children, resolved through existing provider config. "
                "No endpoints/URLs. Without model, uses that provider's default model; tasks[].provider takes precedence.",
            ),
            ...
            "tasks": {
                ...
                "items": {
                    "type": "object",
                    "properties": {
                        "model": _p("string", "Per-task model name, overriding top-level model and delegation.model."),
                        "provider": _p(
                            "string", "Per-task named provider, overriding top-level provider and delegation.provider. "
                            "Resolved through existing provider config; no endpoints/URLs.",
                        ),
                        "goal": _p(
                            "string",
